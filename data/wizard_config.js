...
var DR_WIZARD_TASKS = {
  ...
  dr_critica: [
    {texto:'Ejecutar /dr review sobre el borrador',
     detalle:'El review usa 4 agentes independientes: (1) Crítico de contenido (CT+PL+RM+IA), (2) Humanizer (anti-IA), (3) Verificador (TF), (4) Evaluador de 30 principios de escritura. Cada agente evalúa sin ver a los otros. El review lee automáticamente tu fase activa de CRISOL (vía Supabase) para ajustar la severidad, y tus declaraciones socráticas anteriores para verificar coherencia. Al terminar, escribe alertas a CRISOL si detecta problemas críticos.',
     herramientas:'Claude Code → /dr review [archivo]',
     prompts:['/dr review [RUTA O TEXTO DEL BORRADOR]'],
     skill:'dr_review'},
    {texto:'Revisar alertas en CRISOL',
     detalle:'Después de ejecutar /dr review, vuelve a CRISOL. Si el review detectó problemas críticos (principios violados, deducciones graves, score bajo), aparecerán como alertas rojas/amarillas arriba del wizard. Las alertas rojas (bloqueos) deben resolverse antes de poder pasar el gate.',
     herramientas:'CRISOL → alertas arriba del wizard',
     prompts:[]},
    {texto:'Corregir debilidades identificadas',
     detalle:'Usa los códigos de deducción (CT01-CT06, PL01-PL05, etc.) y los 30 principios violados como guía. No necesitas arreglar todo de una vez — prioriza las 3 de mayor impacto. Los errores recurrentes (mismo código que ronda anterior) tienen penalización ×1.5.',
    ...
    {texto:'Re-ejecutar /dr review para verificar mejora',
     detalle:'El review de ronda 2+ compara automáticamente con la ronda anterior: detecta errores corregidos, errores nuevos, y errores recurrentes. Verifica que el delta sea positivo. Las alertas resueltas desaparecen de CRISOL.',
  ...
  dr_verify: [
    {texto:'Ejecutar /dr verify sobre el texto',
     detalle:'El verificador revisa TODAS las citas contra los PDFs originales. Clasifica errores en 5 tipos: F1-F5. Al terminar, escribe automáticamente alertas a CRISOL vía Supabase: F1-F3 como bloqueos rojos, F4-F5 como advertencias amarillas. El gate de verificación NO se puede saltar — ZERO TOLERANCIA para cualquier defecto sin resolver o declarar.',
  ...
  dr_depth: [
    {texto:'Ejecutar /dr mentor para profundizar',
     detalle:'El mentor socrático lee automáticamente tus declaraciones anteriores (vía Supabase) y NO repite preguntas ya hechas. Hace preguntas nuevas basadas en lo que declaraste en gates anteriores y en los hallazgos del review. Al terminar, guarda el diálogo en Supabase para que CRISOL personalice los próximos gates. Anti-sycophancy: nunca halaga sin sustancia.',
  ...
  dr_benchmark: [
    {texto:'Solicitar propuesta de publicaciones ancla',
     detalle:'Claude analiza las fuentes que ya citas en tu artículo y propone 5-8 publicaciones ancla candidatas, organizadas por tipo: (1) teórica pura del campo, (2) cercana a tu tema, (3) con validación empírica. Para cada una indica: por qué es relevante, cuántas citas tiene, y qué dimensión fortalecería la comparación.',
     herramientas:'Claude Code → /dr benchmark --suggest',
     prompts:['/dr benchmark --suggest\n\nMi artículo trata sobre: [TEMA/CONSTRUCTO PRINCIPAL]\n\nFuentes que ya cito: [LISTAR LAS PRINCIPALES]\n\nPropón 5-8 publicaciones ancla candidatas para benchmarking. Para cada una indica:\n- Autor, año, título, journal\n- Tipo: teórica pura / cercana al tema / empírica\n- Citas aproximadas\n- Por qué sería buena ancla para mi artículo\n- Qué dimensión fortalecería (originalidad, rigor, alcance, etc.)'],
     skill:'dr_benchmark'},
    {texto:'Evaluar y seleccionar 3-4 anclas',
     detalle:'De las anclas propuestas, selecciona 3-4 según tu criterio. Idealmente: 1 teórica pura, 1 cercana a tu tema, 1 con validación empírica. Las anclas descartadas también son valiosas — documentan contra quién decidiste NO compararte y por qué.',
     herramientas:'Tu criterio como investigador',
     prompts:[]},
    {texto:'Descargar PDFs de las anclas seleccionadas',
     detalle:'Busca y descarga los PDFs de las anclas elegidas. Claude necesita acceso al contenido completo para hacer una comparación precisa — no basta con el abstract. Guárdalos en tu carpeta de fuentes.',
     herramientas:'Google Scholar · Sci-Hub · Biblioteca universitaria · DOI.org',
     prompts:[]},
