// ============================================================
// CRISOL — projects-render.js
// Decomposed sub-renders for project dashboard
// Extracted from renderProjectDash (Sprint 7c, Paso 8)
// ============================================================

import { state, DEFAULT_FASES, PHASE_GATES, PROJ_ESTADOS, PROJ_ROLE_LABELS, PROJ_ROLES } from './state.js';
import { escH, linkify } from './utils.js';
...
import { getProjects, saveProjects, canEditProject } from './projects-core.js';
...
import { getActivePhase, renderWizardTask, _getDefaultChecklist } from './projects-phases.js';
import { _getDrFases, _getDrWizardTasks, _getDrPhaseGates, _getArtifactTags, migrateDrFases, loadAndRenderDrAlerts } from './projects-dr.js';

// Local config fallback (wizard tasks loaded as globals)
function _getDefaultWizardTasks() {
  return (typeof DEFAULT_WIZARD_TASKS !== 'undefined') ? DEFAULT_WIZARD_TASKS : {};
}

// ============================================================
// Sub-render: Header (back link + title + meta + team + phase)
// ============================================================
function renderProjectHeader(proj, isOwner, days, projects) {
  ...
  loadProjectMembers(proj.id).then(members => {
    const area = document.getElementById('proj-members-area');
    if (!area || members.length <= 1) return;
    ...
    const isOwnerMember = members.some(m => m.role === 'owner' && m.user_id === state.currentUser?.id);
...
// Sub-render: KPIs
// ============================================================
function renderProjectKPIs(proj, projArts, projDocs, totalWords, days, daysClass) {
  let h = `<div class="proj-kpis">`;
...
// ============================================================
// Sub-render: Kanban tasks
// ============================================================
function renderProjectKanban(proj) {
  ...
  let h = `<div style="margin:12px 0 8px;"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">`;
...
function renderProjectLogbook(proj, isOwner) {
...
// ============================================================
// Sub-render: Decisions
// ============================================================
function renderProjectDecisions(proj, isOwner) {
  const decisiones = proj.decisiones || [];
  let h = `<div class="proj-section-title"><span>⚡ Decisiones clave (${decisiones.length})</span>${isOwner ? `<button onclick="addDecision('${proj.id}')">+ Decisión</button>` : ''}</div>`;
...
// ============================================================
// Sub-render: Resources
// ============================================================
function renderProjectResources(proj, isOwner) {
  const recursos = proj.recursos || [];
  let h = `<div class="proj-section-title"><span>📚 Recursos del proyecto (${recursos.length})</span>${isOwner ? `<button onclick="addProjectResource('${proj.id}')">+ Recurso</button>` : ''}</div>`;
...
// ============================================================
// Sub-render: Prompt log
// ============================================================
function renderProjectPromptLog(proj) {
  const promptLog = proj.promptLog || [];
  let h = `<div class="proj-section-title"><span>🤖 Prompt log (${promptLog.length})</span><button onclick="showPromptLogModal('${proj.id}')">+ Prompt</button></div>`;
...
function renderProjectTimeline(proj) {
  ...
  let h = `<div class="proj-section-title"><span>📊 Timeline</span></div>`;
...
// ============================================================
// Sub-render: Team dashboard + activity feed (async placeholders)
// ============================================================
function renderProjectTeamAsync(projId) {
  let h = `<div id="proj-team-dashboard"></div>`;
...
// ============================================================
// Sub-render: Workflow mode selector + standard phases + wizard
// ============================================================
function renderProjectWorkflow(proj, projArts, projDocs, totalWords, projects) {
  const calcArticleProgress = state._calcArticleProgress;
  const calcArticleClaims = state._calcArticleClaims;

  if (!proj.fases) { proj.fases = JSON.parse(JSON.stringify(DEFAULT_FASES)); saveProjects(projects); }
  const wfm = proj.workflowMode || (proj.drMode ? 'dr' : 'default');
  let h = `<div style="margin:16px 0 8px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">`;
...
// ============================================================
// Sub-render: DR Branches
// ============================================================
function renderProjectBranches(proj) {
  const wfModeB = proj.workflowMode || (proj.drMode ? 'dr' : 'default');
  if (!((wfModeB === 'dr' || wfModeB === 'mixed') && proj.drBranches && proj.drBranches.length > 1)) return '';

  const activeBranch = proj.drActiveBranch || 'main';
  let h = `<details class="pd-section" open><summary><div class="pd-header"><span class="pd-chevron">▶</span><span class="pd-title">🌿 Ramas del proyecto</span><span class="pd-count">${proj.drBranches.length}</span></div></summary><div class="pd-body">`;
  ...
  function renderBranchTree(branch, depth) {
    ...
    const activePhaseB = branchFases.find(f => f.estado === 'en_progreso');
    ...
    if (activePhaseB) bh += ` · Fase: ${activePhaseB.nombre}`;
    bh += `</div>`;
    bh += `</div>`;
...
// ============================================================
// Sub-render: DR Wizard
// ============================================================
function renderProjectDrWizard(proj, projects) {
  ...
  let h = `<div style="margin:8px 0 12px;">`;
  ...
  // DR Wizard Guide for active phase
  if (drActive) {
...
// ============================================================
// Sub-render: CLO Wizard
// ============================================================
function renderProjectCloWizard(proj, projects) {
  const wfMode = proj.workflowMode || (proj.drMode ? 'dr' : 'default');
  const isDrMode = wfMode === 'dr' || wfMode === 'mixed';
...
// ============================================================
// Sub-render: Artifacts registry
// ============================================================
function renderProjectArtifacts(proj) {
  const artifacts = proj.drArtifacts || [];
  const wfMode = proj.workflowMode || (proj.drMode ? 'dr' : 'default');
  if (!(wfMode === 'dr' || wfMode === 'mixed' || wfMode === 'clo')) return '';

  const tags = _getArtifactTags();
  const hasScored = artifacts.filter(a => a.score != null).length >= 2;

  let h = `<details class="pd-section"${artifacts.length > 0 ? ' open' : ''}>`;
...
// ============================================================
// Sub-render: External links + folder structure
// ============================================================
function renderProjectLinks(proj) {
  const links = proj.links || [];
  let h = `<details class="pd-section"${links.length > 0 ? ' open' : ''}><summary><div class="pd-header"><span class="pd-chevron">▶</span><span class="pd-title">Links del proyecto</span><span class="pd-count">${links.length}</span></div></summary><div class="pd-body">`;
...
// ============================================================
// Sub-render: Pre-submission checklist
// ============================================================
function renderProjectChecklist(proj, projects) {
  ...
  const headerPhase = proj.fases ? proj.fases.find(f => f.estado === 'en_progreso') : null;
  const isRevisionPhase = headerPhase && (headerPhase.id === 'revision' || headerPhase.id === 'submission');
  let h = `<details class="pd-section"${isRevisionPhase ? ' open' : ''}><summary><div class="pd-header"><span class="pd-chevron">▶</span><span class="pd-title">Checklist pre-submission</span><span class="pd-count">${checkDone}/${checkApplicable} (${checkPct}%)</span></div></summary><div class="pd-body">`;
  h += `<div style="margin-bottom:12px;">`;
...
// ============================================================
// Sub-render: Research question evolution log
// ============================================================
function renderProjectQuestionLog(proj) {
  const qLog = proj.preguntaLog || [];
  const headerPhase = proj.fases ? proj.fases.find(f => f.estado === 'en_progreso') : null;
  const isIdeacionPhase = headerPhase && (headerPhase.id === 'ideacion' || headerPhase.id === 'fundamentacion');
  let h = `<details class="pd-section"${isIdeacionPhase || qLog.length > 0 ? ' open' : ''}><summary><div class="pd-header"><span class="pd-chevron">▶</span><span class="pd-title">Evolución de la pregunta de investigación</span><span class="pd-count">${qLog.length}</span></div></summary><div class="pd-body">`;
...
// ============================================================
// Sub-render: Articles section
// ============================================================
function renderProjectArticles(proj, projArts, isOwner) {
  const calcArticleProgress = state._calcArticleProgress;
  const calcArticleClaims = state._calcArticleClaims;

  let h = `<div class="proj-section-title"><span>${proj._isShared ? 'Fuentes del proyecto' : 'Mis fuentes'} (${projArts.length})</span>${isOwner ? `<button onclick="addArticleToProject('${proj.id}')">+ Agregar</button>` : ''}</div>`;
...
// ============================================================
// Sub-render: Documents section
// ============================================================
function renderProjectDocs(proj, projDocs, isOwner) {
  const calcDocBlockStates = state._calcDocBlockStates;

  let h = `<div class="proj-section-title"><span>${proj._isShared ? 'Escritos del proyecto' : 'Mis escritos'} (${projDocs.length})</span>${isOwner ? `<div style="display:flex;gap:6px;"><button onclick="createDocFromProject('${proj.id}')">+ Crear</button><button onclick="addDocToProject('${proj.id}')">+ Agregar</button></div>` : ''}</div>`;
...
// ============================================================
// Sub-render: Source→Section map
// ============================================================
function renderProjectSections(proj, projArts) {
  const secciones = proj.secciones || [];
  const headerPhase = proj.fases ? proj.fases.find(f => f.estado === 'en_progreso') : null;
  const isEscrituraPhase = headerPhase && (headerPhase.id === 'escritura' || headerPhase.id === 'fundamentacion');
  let h = `<details class="pd-section"${isEscrituraPhase && secciones.length > 0 ? ' open' : ''}><summary><div class="pd-header"><span class="pd-chevron">▶</span><span class="pd-title">Mapa fuentes → secciones</span><span class="pd-count">${secciones.length}</span></div></summary><div class="pd-body">`;
...
// Sub-render: Claims
// ============================================================
function renderProjectClaims(proj, projArts) {
  if (projArts.length === 0) return '';

  const getProjectClaims = state._getProjectClaims;
  ...
  const headerPhase = proj.fases ? proj.fases.find(f => f.estado === 'en_progreso') : null;
  const isEscrituraPhase = headerPhase && (headerPhase.id === 'escritura' || headerPhase.id === 'fundamentacion');
  let h = `<details class="pd-section"${isEscrituraPhase && totalClaims > 0 ? ' open' : ''}><summary><div class="pd-header"><span class="pd-chevron">▶</span><span class="pd-title">Claims del proyecto</span><span class="pd-count">${totalClaims}</span></div></summary><div class="pd-body">`;
...
// ============================================================
// renderProjectDash — Orchestrator
// ============================================================
function renderProjectDash(projId) {
  ...
  const days = state._calcDaysRemaining(proj.fechaLimite);
  const daysClass = days < 7 && days !== Infinity ? 'urgent' : days < 14 && days !== Infinity ? 'warning' : '';
  const isOwner = canEditProject(proj);

  // Build HTML from sub-renders
  let h = '';
  h += renderProjectHeader(proj, isOwner, days, projects);
  h += renderProjectKPIs(proj, projArts, projDocs, totalWords, days, daysClass);
  h += renderProjectKanban(proj);
  h += renderProjectLogbook(proj, isOwner);
  h += renderProjectDecisions(proj, isOwner);
  h += renderProjectResources(proj, isOwner);
  h += renderProjectPromptLog(proj);
  h += renderProjectTimeline(proj);
  h += renderProjectTeamAsync(proj.id);
  h += renderProjectWorkflow(proj, projArts, projDocs, totalWords, projects);
  h += renderProjectBranches(proj);
  h += renderProjectDrWizard(proj, projects);
  h += renderProjectCloWizard(proj, projects);
  h += renderProjectArtifacts(proj);
  h += renderProjectLinks(proj);
  h += renderProjectChecklist(proj, projects);
  h += renderProjectQuestionLog(proj);
  h += renderProjectArticles(proj, projArts, isOwner);
  h += renderProjectDocs(proj, projDocs, isOwner);
  h += renderProjectSections(proj, projArts);
  h += renderProjectClaims(proj, projArts);
...
// ============================================================
// renderPipeline — Pipeline view (moved from projects.js)
// ============================================================
function renderPipeline() {
  const ct = document.getElementById('ct');
  const projects = getProjects();
  const calcDaysRemaining = state._calcDaysRemaining;
...
function renderProjectsSummary() {
  ...
  const calcDaysRemaining = state._calcDaysRemaining;
  const calcDaysElapsed = state._calcDaysElapsed;
  const calcProjectAlert = state._calcProjectAlert;
...
// ============================================================
// renderMiTesis — cross-article claims grouped
// ============================================================
